...
void AngleAcceleration_Digital_init(void)
{
  ...
  gpio_init (PTB7, GPO,HIGH);     //初始化I2C时钟引脚
  gpio_init (PTB10, GPO,HIGH);    //初始化I2C数据引脚
...
void AngleAcceleration_Digital_Data(uint16_t *AdColle)
{
  ...
  AdColle[0] = ad_once(ADC0_SE4a, ADC_12bit) ;  //AR2
  AdColle[1] = ad_once(ADC0_SE7a, ADC_12bit) ;  //AR1
  AdColle[2] = ad_once(ADC0_SE3, ADC_12bit) ;  //AR3  
}
...
uint8 AngleAcceleration_AD(uint16_t *AdColle,uint8_t Count)
{
   uint8  BCount ;
   BCount = Count ;      
     AdColle[0] += ad_sum(ADC0_SE4a, ADC_12bit, ADCOUT);  //AR2
     AdColle[1] += ad_sum(ADC0_SE7a, ADC_12bit, ADCOUT);  //AR1
     AdColle[2] += ad_sum(ADC0_SE3,  ADC_12bit, ADCOUT);  //AR3
     AdColle[3] += ad_sum(ADC0_SE14, ADC_12bit, ADCOUT);  //ZG源程序
     AdColle[4] += ad_sum(ADC0_SE8,  ADC_12bit, ADCOUT);  //YG源程序
     AdColle[5] += ad_sum(ADC0_SE0, ADC_12bit, ADCOUT);//xG源程序 
